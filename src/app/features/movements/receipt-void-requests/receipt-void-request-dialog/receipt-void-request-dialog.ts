...
export interface ReceiptVoidRequestDialogData {
  receiptId: number;
}

@Component({
  selector: 'app-receipt-void-request-dialog',
  imports: [ReactiveFormsModule, Dialog, AppInput, Select, TranslatePipe],
  templateUrl: './receipt-void-request-dialog.html',
})
export class ReceiptVoidRequestDialog {
  private readonly data = inject<ReceiptVoidRequestDialogData>(DIALOG_DATA);
  private readonly dialogRef = inject(DialogRef<boolean, ReceiptVoidRequestDialog>);
  ...
  protected async onSave(): Promise<void> {
    if (!this.form.controls.voidReasonId.value) {
      this.toastService.show(this.languageService.t('receiptVoidRequests.dialog.reasonRequired'));
      return;
    }

    this.saving.set(true);
    try {
      await this.receiptVoidRequestService.create({
      ...
      this.toastService.show(this.languageService.t('receiptVoidRequests.dialog.success'));
      this.dialogRef.close(true);
    } catch (error) {
      const message = (error as { error?: { messages?: { description: string }[] } })?.error?.messages?.[0]
        ?.description;
      this.toastService.show(this.languageService.t('receiptVoidRequests.dialog.error'), message);
    } finally {
